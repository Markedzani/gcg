const CLICK_SOUND_PATH = "./Sounds/clicksoundeffect.mp3";
const BUY_SOUND_PATH = "./Sounds/ka-ching.mp3"
...
setTimeout(function () { document.getElementById("counter").textContent = "cash: " + nicenums(cash) }, 1)

function nicenums(number) {
    if (!(number < 1000)) {
        ...
        let formattedNumber;
        if (isNegative) {
            formattedNumber = (number >= 999.99 || suffixIndex === 0 ? '-' : '') + number.toFixed(2) + suffixes[suffixIndex];
        } else {
            formattedNumber = (number >= 999.99 && suffixIndex !== 0 ? Math.floor(number) : number.toFixed(2)) + suffixes[suffixIndex];
        }
...
function tickled() {
    if (!(paused)) {
        ...
        cash += cpc
        document.getElementById("counter").textContent = "cash: " + nicenums(cash)
        console.log(cash)
...
function CPC(amount, loss /* amount = CPC += num, loss = cash -= num */) {
    if (!(paused)) {
        if (cash >= loss) {
            ...
            cpc += amount
            document.getElementById("counter").textContent = "cash: " + nicenums(cash)
            document.getElementById("CpC").textContent = "[ +" + nicenums(cpc) + "$/click ]"

            const BUY_AUDIO = new Audio(BUY_SOUND_PATH)
            BUY_AUDIO.play()
        } else {
            mistake("Upgrade failed, you need " + nicenums((loss - cash)) + "$ more!")
        }
    }
}
    ...
    if (!(aMaxed)) {
        if (!(paused)) {
            if (cash >= aCost) {
                ...
                document.getElementById("-.01AUTO").textContent = "-0.01⏰ $" + nicenums(aCost)
...
async function buyfood() {
    if (!(paused)) {
        if (!(boughtMaid)) {
            ...
            if (cash >= 150) {
                const ka_ching = new Audio(BUY_SOUND_PATH)
                ka_ching.play()
...
async function mistake(txt) {
    if (!(paused)) {
        ...
        const audio = new Audio("./Sounds/windowsXPerror.mp3")
        audio.play()
        await delay(1000)
...
async function buymaid() {
    ...
    if (!(paused)) {
        ...
        if (cash >= 15000) {
            ...
            if (!(boughtMaid)) {
                mistake("not enough. yu need " + nicenums(15000 - cash) + "$")
            } else {
                mistake("you already bought d' maid lol")
            }
        }
...
function theme() {
    ...
    if (themee) {
        ...
        document.getElementById("GB").src = "./Images/gingerbreadman.png"
        btn.color = "white"
        cnt.style.color = "#000"
    } else {
        ...
        document.getElementById("GB").src = "./Images/Tgingie.png"
